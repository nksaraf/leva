import React, { useEffect, useLayoutEffect, useRef, useState, VFC, Suspense } from 'react'
...
const ShowAxes = () => {
  const [showAxes] = useWorldStore((state) => [state.showAxes], shallow)
  return showAxes ? <axesHelper args={[500]} /> : null
}
...
const EditorControls = () => {
  ...
  const [selectedHdr, useHdrAsBackground, showGrid, showAxes, setOrbitControlsRef] = useWorldStore(
    (state) => [state.selectedHdr, state.useHdrAsBackground, state.showGrid, state.showAxes, state.setOrbitControlsRef],
    shallow
  )
            ...
            {sceneSnapshot ? (
              ...
              <div className="flex justify-center items-center bg-white h-screen">
                <div className="flex flex-col gap-5 items-center ">
                  <Heading className="text-2xl mb-4">No canvas connected</Heading>
                  <div>
                    Please use <Code>configure()</Code> and <Code>bind()</Code> to connect a canvas to React Three
                    Editable.
                  </div>
                  <Code block>
