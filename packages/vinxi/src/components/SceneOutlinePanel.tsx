...
import { folder, Leva, useControls, useCreateStore } from 'leva'
...
const ObjectButton: VFC<ObjectButtonProps> = ({ objectName, editableType, selected, ...props }) => {
  ...
  return (
    <ButtonImpl
      ...
      className={`inline-flex justify-start items-center rounded-md px-4 py-2 font-medium focus:outline-none focus:ring focus:ring-blue-300 ${
        objectName === selected ? 'bg-green-800 hover:bg-green-900 text-white' : 'text-gray-700 hover:bg-gray-200'
      }`}>
...
const SceneOutlinePanel: VFC = () => {
  const [editablesSnapshot, selected, setSelected, createSnapshot] = useWorldStore(
    state => [state.editablesSnapshot, state.selected, state.setSelected, state.createSnapshot],
    shallow
  )
  ...
  useControls(
    {
      items: folder(Object.fromEntries(Object.entries(editablesSnapshot).map(([name, editable]) => [name, true]))),
    },
    { store }
  )
