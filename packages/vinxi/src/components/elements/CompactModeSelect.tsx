...
import { Tooltip, TooltipReference, usePopoverState, useTooltipState, PopoverDisclosure, Popover } from '.'
...
const OptionButton = function <Option>({ value, option, label, icon, onClick }: OptionButtonProps<Option>) {
...
const Settings: VFC<SettingsProps> = ({ children }) => {
  ...
  return (
    <>
      <TooltipReference {...tooltip} as={'div'} tabIndex={-1} className="focus:outline-none">
        <PopoverDisclosure
          // @ts-ignore
          {...popover}
...
const CompactModeSelect = function <Option extends string | number>({
  value,
  onChange,
  options,
  settingsPanel,
